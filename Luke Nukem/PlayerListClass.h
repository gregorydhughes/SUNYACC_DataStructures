...
typedef PlayerRec PlayerType;	//simple data teyp for testing of methods

class PlayerListClass
{

public:
	PlayerListClass();			//default constructor 
	...
	void Insert(PlayerType item);	//item to be added
	...
	bool Find(PlayerType item);// item to be found
	...
	PlayerType Retrieve() const;
	//Purpose:	Returns the current item in the list
	//Pre:		EndOfList is false
	//Post:		Returns the item at currPos

	virtual PlayerListClass operator = (const PlayerListClass& orig);	//list to be copied
	...
	void UpdateItem(PlayerType item);
...
private:
	PlayerType list[MAX_LIST_SIZE];		//stores the items in the list
